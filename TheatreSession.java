...
public class TheatreSession implements XmlSerializable, XmlUnserializable<Integer> {
    ...
    private Seat[] findRandomFit(SeatType type, int seats) throws IllegalArgumentException {
        ...
        for (int spanLength = Math.min(theatre.getWidth(), remainingSeats); remainingSeats > 0; ) {
            // Don't decrease spanLength until we know that it will no longer
            // fit!
            while (remainingSeats >= spanLength && remainingSeats > 0) {
                
                Seat[] allocation = new Seat[0];
                
                if (spanLength == 1) {
                    for(int i = 0; i < rows.length; i++) {
                        for(int j = 0; j < rows[i].length; j++) {
                            if(rows[i][j].getState() == SeatState.Held) {
                                if(j > 0 && rows[i][j-1].getType() == type && rows[i][j-1].getState() == SeatState.Empty) {
                                    allocation = new Seat[1];
                                    allocation[0] = rows[i][j-1];
                                    break;
                                }
                                if(j < rows[i].length-1 && rows[i][j+1].getType() == type && rows[i][j+1].getState() == SeatState.Empty) {
                                    allocation = new Seat[1];
                                    allocation[0] = rows[i][j+1];
                                    break;
                                }
                ...
                if(allocation.length == 0)
                    allocation = getContiguousAllocation(rows, type, spanLength);
...
//                        spanLength = spanLength/2 + 1;
                ...
                spanLength = Math.min(theatre.getWidth(), remainingSeats);
            }
        }
